...
def euler2matrix(input_angle,rotate_seq):
  ...
  #if rotate_seq=='zyz':
  #  Z_phi=np.matrix([[math.cos(phi),math.sin(phi),0],[-math.sin(phi),math.cos(phi),0],[0,0,1]])
  #  N_theta=np.matrix([[1,0,0],[0,math.cos(theta),math.sin(theta)],[0,-math.sin(theta),math.cos(theta)]])
  #  Z_psi=np.matrix([[math.cos(theta),math.sin(theta),0],[-math.sin(theta),math.cos(theta),0],[0,0,1]])
  #  R=Z_phi*N_theta*Z_psi
  #R=N_theta*Z_phi*Z_psi
  #inv_R=R.I
  #  R=R.I
...
for vi in range(8):
  ...
  R=euler2matrix(euler[vi],'zxz')
  print(np.matmul(R,R.T))
  cdx,cdy,cdz=int(coords[vi,0]-start_x)+radius,int(coords[vi,1]-start_y)+radius,int(coords[vi,2]-start_z)+radius
  ...
  print('Writing ...'+str(vi+1)+'/ 8')
  M.write_pix_file(output_volume,'sub'+str(vi)+'.mrc',1)

'''
angle=0,Pi/2,0
R=euler2matrix(angle,'xzx')
output_volume=np.zeros([2*radius,2*radius,2*radius],dtype=ori_volume.dtype)
vector3=np.zeros([1,3],dtype=np.float32)
o_vec=np.zeros([1,3],dtype=np.float32)

calc[blockdim,griddim](ori_volume,R,output_volume,vector3,o_vec)
M.write_pix_file(output_volume,'r.mrc',1)

#'''
#M.write_pix_file(output_full_volume,'full.mrc',1)
#print(time.time()-t1)
